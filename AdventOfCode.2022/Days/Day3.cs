...
{
    private const string PriorityList = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
    ...
    public Solution Solve()
    {
        ...
        var partOne = input.Select(GetCompartiments)
            .SelectMany(GetOverlappingItems)
            .Sum(GetPriority);

        var partTwo = CombineRucksacks(input)
            .Select(GetRepeatingItem)
            .Sum(i => i.HasValue ? GetPriority(i.Value) : 0);

        return new Solution(Day, partOne.ToString(), partTwo.ToString());
    }

    private static int GetPriority(char item)
    {
        return PriorityList.IndexOf(item) + 1;
    }

    private static string[] GetCompartiments(string rucksack)
    {
        return new[] { rucksack[..(rucksack.Length / 2)], rucksack[^(rucksack.Length / 2)..] };
    }

    private static char[] GetOverlappingItems(string[] compartiments)
    {
        return compartiments
            .First()
            .Intersect(compartiments.Last())
            .ToArray();
    }

    private static string[][] CombineRucksacks(string[] input)
    {
        return input.Select((rucksack, index) => new { rucksack, index })
            .GroupBy(i => i.index / 3, i => i.rucksack)
            .ToDictionary(i => i.Key, i => i.ToArray())
            .Values
            .ToArray();
    }

    private static char? GetRepeatingItem(string[] rucksacks)
    {
        return rucksacks[0].FirstOrDefault(r => rucksacks[1].Contains(r) && rucksacks[2].Contains(r));
    }

    private const string RawInput = @"dWlhclDHdFvDCCDfFq
